...
void delay_us(uint32_t microseconds) {
    uint32_t ticks = get_microseconds();
    while (get_microseconds() < ticks + microseconds);
}
/* USER CODE END 0 */
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  hrtc1.Init.AsynchPrediv = 31;
  hrtc1.Init.SynchPrediv = 1023;
  ...
  */
  ...
  if (HAL_RTC_SetAlarm_IT(&hrtc1, &sAlarm, RTC_FORMAT_BIN) != HAL_OK)
  {
    Error_Handler();
  }
